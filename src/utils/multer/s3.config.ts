import { DeleteObjectCommand, DeleteObjectCommandOutput, DeleteObjectsCommand, DeleteObjectsCommandOutput, GetObjectAclCommand, GetObjectCommand, GetObjectCommandOutput, ListObjectsV2Command, ObjectCannedACL, PutObjectCommand, S3Client } from "@aws-sdk/client-s3";
import { StorageEnum } from "./cloud.multer";
import { createReadStream } from "node:fs";
import { v4 as uuid } from 'uuid';
import { BadRequestException } from "../response/error.reponse";
...
export const uploadFile = async ({
  ...
  if (!command?.input?.Key) {
    throw new BadRequestException("fail to generate uploud key")
  }
