...
const decodeToken = async ({ authorization, tokenType = TokenEnum.access, }) => {
    ...
    if (!bearerKey || !token) {
        throw new error_reponse_1.UnauthorizedException("missing token parts");
    }
    ...
    if (!decoded?._id || !decoded?.iat) {
        throw new error_reponse_1.BadRequestException("Invalid token payload");
    }
    if (await tokenModel.findOne({ filter: { jti: decoded.jti } })) {
        throw new error_reponse_1.UnauthorizedException("invalid or old login credentials");
    }
    const user = await userModel.findOne({ filter: { _id: decoded._id } });
    if (!user) {
        throw new error_reponse_1.BadRequestException("Not register account");
    }
...
const createRevokeToken = async (decoded) => {
    ...
    if (!result) {
        throw new error_reponse_1.BadRequestException("Fail to revoke this token");
    }
    return result;
};
