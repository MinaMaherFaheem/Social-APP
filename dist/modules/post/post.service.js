...
const postAvailability = (req) => {
    return [
        { availability: post_model_2.AvailabilityEnum.public },
        {
            availability: post_model_2.AvailabilityEnum.onlyMe,
            createdBy: req.user?._id
        },
        {
            availability: post_model_2.AvailabilityEnum.friends,
            createdBy: { $in: [...(req.user?.friends || []), req.user?._id] }
        },
        {
            availability: { $ne: post_model_2.AvailabilityEnum.onlyMe },
...
class PostService {
    ...
    postModel = new repository_1.PostRepository(post_model_2.PostModel);
    constructor() { }
    createPost = async (req, res) => {
        ...
        if (!post) {
            ...
            throw new error_reponse_1.BadRequestException("fail to create this post");
        }
    ...
    likePost = async (req, res) => {
        ...
        if (!post) {
            throw new error_reponse_1.NotFoundException("invalid postId or post nit exist");
        }
        if (action != post_model_1.LikeActionEnum.unlike) {
            (0, geteway_1.getIo)().to(geteway_1.connectedSockets.get(post.createdBy.toString())).emit("likePost", { postId, userId: req.user?._id });
        }
        return (0, success_response_1.successResponse)({ res });
    };
